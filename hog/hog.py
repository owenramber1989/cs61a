...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    idx = 0
    for i in range(10):
        averaged_dice = make_averaged(roll_dice,total_samples)(i+1,dice)
        if(averaged_dice>ans):
            ans = averaged_dice
            idx = i+1
    return idx
    # END PROBLEM 9
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    a = square_update(0,score,opponent_score,dice=six_sided)
    a -= score
    if (a >= threshold):
        return 0
    else:
